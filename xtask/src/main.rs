...
enum Xtask {
    ...
    #[command(about = "generate SDK and CLI")]
    Generate {
        #[clap(long)]
        check: bool,
        #[clap(short = 'v', long)]
...
fn generate(check: bool, verbose: bool) -> Result<(), String> {
    ...
    spec_path.push("support-api-spec.json");
    ...
    std::io::stdout().flush().unwrap();
    let code = generator.cli(&spec, "support_sdk").unwrap().to_string();
    let contents = format_code(format!("{}\n\n{}", "use support_sdk::*;", code));

    let mut out_path = root_path;
    out_path.push("support-cli");
    out_path.push("src");
    out_path.push("generated");
    ...
    if check {
        ...
        if checked_in != contents {
            ...
            result = Err(format!(
                "{} is out of date relative to support-api-spec.json",
                out_path.to_string_lossy(),
            ));
        } else {
            println!("👍");
